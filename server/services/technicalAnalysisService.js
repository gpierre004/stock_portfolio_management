...
class TechnicalAnalysisService {
  ...
  _interpretSupportResistance(support, resistance) {
    ...
    if (Math.abs(support.distance) < Math.abs(resistance.distance)) {
      return { 
        signal: 'BUY', 
        message: `Near support level at $${support.level} (${Math.abs(support.distance)}% away)` 
      };
    } else {
      return { 
        signal: 'SELL', 
        message: `Near resistance level at $${resistance.level} (${Math.abs(resistance.distance)}% away)` 
      };
    }
  }
}
