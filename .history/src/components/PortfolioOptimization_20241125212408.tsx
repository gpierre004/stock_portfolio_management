...
export default function PortfolioOptimization({ accountId }: PortfolioOptimizationProps) {
  ...
  const { data: holdings = [], isLoading, error } = useQuery<PortfolioSummary[]>({
  ...
  if (error) {
    return (
      <div className="bg-red-50 text-red-600 p-4 rounded-lg">
        An error occurred while loading portfolio data. Please try again.
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        {rebalanceMutation.error && (
          <div className="mt-4 text-red-600">
            Failed to calculate rebalancing suggestions. Please try again.
          </div>
        )}
      </div>
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        {scenarioMutation.error && (
          <div className="mt-4 text-red-600">
            Failed to calculate scenario analysis. Please try again.
          </div>
        )}
      </div>
