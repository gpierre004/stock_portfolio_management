...
interface Indicators {
  ...
  supportResistance: {
    support: Array<{ level: number; touches: number }>;
    resistance: Array<{ level: number; touches: number }>;
  };
}
    ...
    <div className="bg-white shadow rounded-lg p-6">
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded p-3">
              ...
              <div className="text-lg font-semibold">{rsi.toFixed(2)}</div>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <span className="text-sm text-gray-500">MACD</span>
              <div className="text-lg font-semibold">{macd.value.toFixed(2)}</div>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <span className="text-sm text-gray-500">MA (20)</span>
              <div className="text-lg font-semibold">${movingAverages.ma20.toFixed(2)}</div>
            </div>
            <div className="bg-gray-50 rounded p-3">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-50 rounded p-3">
              <span className="text-sm text-gray-500">Support Levels</span>
              <div className="space-y-2 mt-2">
                {supportResistance.support.map((level, index) => (
                  <div key={`support-${index}`} className="flex justify-between items-center">
                    <span className="text-sm font-medium">${level.level.toFixed(2)}</span>
                    <span className="text-xs text-gray-500">{level.touches} touches</span>
                  </div>
                ))}
                {supportResistance.support.length === 0 && (
                  <div className="text-sm text-gray-500">No support levels detected</div>
                )}
              </div>
            </div>
            <div className="bg-gray-50 rounded p-3">
              <span className="text-sm text-gray-500">Resistance Levels</span>
              <div className="space-y-2 mt-2">
                ...
                {supportResistance.resistance.length === 0 && (
                  <div className="text-sm text-gray-500">No resistance levels detected</div>
                )}
              </div>
            </div>
          </div>
