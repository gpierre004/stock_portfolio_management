...
class TechnicalAnalysisService {
  async calculateSupportResistance(ticker) {
    const query = `
      SELECT 
        current_price,
        support_price,
      ...
      FROM public.vw_support_resistance_levels
      WHERE ticker = $1
      AND date = (SELECT MAX(date) FROM public.vw_support_resistance_levels)
    `;

    const result = await pool.query(query, [ticker]);
    const levels = result.rows[0] || null;

    if (levels) {
      return {
        support: [{ level: levels.support_price, distance: levels.distance_to_support }],
        resistance: [{ level: levels.resistance_price, distance: levels.distance_to_resistance }]
      };
    }
  ...
  async generateTradingSignals(ticker) {
    ...
    const signals = {
      ...
      signals: {
        ...
        supportResistance: this._interpretSupportResistance(levels?.support[0], levels?.resistance[0])
      }
    };
  ...
  _interpretSupportResistance(support, resistance) {
    if (!support || !resistance) {
      return { signal: 'NEUTRAL', message: 'No clear support/resistance levels' };
    }

    if (Math.abs(support.distance) < Math.abs(resistance.distance)) {
      return { 
        signal: 'BUY', 
        message: `Near support level at $${support.level.toFixed(2)} (${Math.abs(support.distance).toFixed(2)}% away)` 
      };
    } else {
      return { 
        signal: 'SELL', 
        message: `Near resistance level at $${resistance.level.toFixed(2)} (${Math.abs(resistance.distance).toFixed(2)}% away)` 
      };
    }
  }
}
