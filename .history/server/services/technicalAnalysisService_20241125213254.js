...
class TechnicalAnalysisService {
  async calculateSupportResistance(ticker, period = 20) {
    const query = `
      ...
      pivots AS (
        SELECT
          ...
          date,
          CASE 
            WHEN close > prev_close THEN high
            ELSE low
          END as pivot_point
        FROM price_levels
      ...
      resistance_levels AS (
        SELECT 
          ticker,
          pivot_point as level,
          COUNT(*) as touches
        FROM pivots
        GROUP BY ticker, pivot_point
        HAVING COUNT(*) >= 2
        ORDER BY pivot_point DESC
        LIMIT 3
      ),
      support_levels AS (
        SELECT 
        ...
        GROUP BY ticker, pivot_point
        HAVING COUNT(*) >= 2
        ORDER BY pivot_point ASC
        LIMIT 3
      )
      SELECT 
        COALESCE(json_agg(DISTINCT r), '[]') as resistance,
        COALESCE(json_agg(DISTINCT s), '[]') as support
      FROM resistance_levels r
      CROSS JOIN support_levels s;
    `;

    const result = await pool.query(query, [ticker, period]);
  ...
  async calculateMACD(ticker) {
    const query = `
      ...
      SELECT 
        ...
        AVG(ema_12 - ema_26) OVER (ORDER BY date ROWS BETWEEN 8 PRECEDING AND CURRENT ROW) as signal_line
      FROM ema_calc
    ...
    const query = `
      WITH volume_stats AS (
        SELECT 
          ...
          STDDEV(volume) OVER (PARTITION BY ticker ORDER BY date ROWS BETWEEN 19 PRECEDING AND CURRENT ROW) as volume_stddev
        FROM stock_prices
        WHERE ticker = $1
        ORDER BY date DESC
        LIMIT 20
  ...
  async generateTradingSignals(ticker) {
    ...
    const signals = {
      ...
      signals: {
        ...
        supportResistance: this._interpretSupportResistance(levels?.support, levels?.resistance, ma?.price)
      }
    };
  ...
  _interpretSupportResistance(support, resistance, currentPrice) {
    if (!support || !resistance || !currentPrice) {
      return { signal: 'NEUTRAL', message: 'Insufficient data' };
    }

    const nearestSupport = support[0]?.level;
    const nearestResistance = resistance[0]?.level;

    if (!nearestSupport || !nearestResistance) {
      return { signal: 'NEUTRAL', message: 'No clear support/resistance levels identified' };
    }
    ...
    if (distanceToSupport < 2) {
      return { signal: 'BUY', message: `Near support level at $${nearestSupport.toFixed(2)} (${distanceToSupport.toFixed(2)}% away)` };
    } else if (distanceToResistance < 2) {
      return { signal: 'SELL', message: `Near resistance level at $${nearestResistance.toFixed(2)} (${distanceToResistance.toFixed(2)}% away)` };
    }

    return {
      signal: 'NEUTRAL',
      message: `Support at $${nearestSupport.toFixed(2)}, Resistance at $${nearestResistance.toFixed(2)}`
    };
  }
}
