...
class TechnicalAnalysisService {
    ...
    static async getBreakoutStocks() {
        ...
        try {
            logger.info('Starting getBreakoutStocks...');
            ...
            `);
            logger.info('View existence check:', viewCheck.rows[0].exists);

            if (!viewCheck.rows[0].exists) {
                throw new Error('View vw_potential_breakouts does not exist');
            }

            // Get the breakout stocks
            ...
            if (result.rows.length > 0) {
                logger.info('Sample breakout stock:', result.rows[0]);
            }
    ...
    static async getBreakoutSummary() {
        const client = await db.connect();
        try {
            logger.info('Starting getBreakoutSummary...');
            const result = await client.query(`
                SELECT 
                    trend_status,
                    COUNT(*) as count,
                ...
                ORDER BY 
                    ...
                    END
            `);
            logger.info(`Found summary data with ${result.rows.length} trend categories`);
            return result.rows;
        } catch (error) {
    ...
    static async getStockAnalysis(symbol) {
        const client = await db.connect();
        try {
            logger.info(`Starting getStockAnalysis for symbol: ${symbol}`);
            const result = await client.query(
            ...
            logger.info(`Found analysis data for ${symbol}`);
            return {
                ...result.rows[0],
                analysis_details: {
                    moving_averages: {
    ...
    static async getTradingSignals(symbol) {
        const client = await db.connect();
        try {
            logger.info(`Starting getTradingSignals for symbol: ${symbol}`);
            ...
            logger.info(`Successfully calculated indicators for ${symbol}`);
            return {
                ticker: symbol,
