    ...
    const query = `
      WITH price_changes AS (
        SELECT 
          ...
          timestamp,
          price,
          price - LAG(price) OVER (PARTITION BY ticker ORDER BY timestamp) as price_change
        FROM stock_prices
        WHERE ticker = $1
      ...
      avg_gains_losses AS (
        SELECT 
          ticker,
          timestamp,
          AVG(gain) OVER (ORDER BY timestamp ROWS BETWEEN $3-1 PRECEDING AND CURRENT ROW) as avg_gain,
          AVG(loss) OVER (ORDER BY timestamp ROWS BETWEEN $3-1 PRECEDING AND CURRENT ROW) as avg_loss
        FROM gains_losses
      )
      SELECT 
        ticker,
